...
/*
    ...
    my_phpMyAdmin : rename_table.php
    File description:
        REST API to rename a table.
*/
...
if ($database) {
    ...
    if ($db) {
        ...
        if ($data->database_name && $data->table_name && $data->new_name) {
            $manager = new ManageTable($db, $data->database_name);
            $result = $manager->rename_table($data->table_name, 
                $data->new_name);
            if ($result) {
                $response_code = 200;
